...
def _collect_project_sessions(messages: List[TranscriptEntry]) -> List[Dict[str, Any]]:
    ...
    # Convert to list format with formatted timestamps (skip empty sessions)
...
def process_selected_projects(
    selected_project_dirs: List[Path],
    from_date: Optional[str] = None,
    to_date: Optional[str] = None,
    use_cache: bool = True,
    generate_individual_sessions: bool = True,
    ...
    """Process only selected project directories and create linked HTML files.

    Args:
        selected_project_dirs: List of project directory paths to process
        from_date: Optional start date filter
        to_date: Optional end date filter
        use_cache: Whether to use cache for performance
        generate_individual_sessions: Whether to generate individual session HTML files
        output_dir: Optional output directory. If provided, saves all HTML files here.
                   If not provided, saves to parent of first project directory.

    Returns:
        Path to generated index.html file
    """
    if not selected_project_dirs:
        raise ValueError("No project directories selected")

    # Determine base projects path
    projects_path = selected_project_dirs[0].parent

    # Call process_projects_hierarchy with selected directories
    return process_projects_hierarchy(
        ...
        selected_dirs=selected_project_dirs,
    )
...
def process_projects_hierarchy(
    ...
    generate_individual_sessions: bool = True,
    output_dir: Optional[Path] = None,
    selected_dirs: Optional[List[Path]] = None,
) -> Path:
    ...
    Args:
        ...
        generate_individual_sessions: Whether to generate individual session HTML files
        output_dir: Optional output directory. If provided, saves all HTML files here.
                   If not provided, saves to projects_path.
        selected_dirs: Optional list of specific project directories to process.
                      If not provided, processes all directories.
    """
    if not projects_path.exists():
        raise FileNotFoundError(f"Projects path not found: {projects_path}")

    # Find project directories to process
    if selected_dirs:
    ...
    base_output_dir = output_dir if output_dir else projects_path
    ...
    for project_dir in sorted(project_dirs):
        try:
            ...
            # Phase 2: Determine output path for this project
            # If custom output_dir specified, create project subdirectory
            if output_dir:
                project_output_dir = base_output_dir / project_dir.name
                project_output_dir.mkdir(parents=True, exist_ok=True)
                project_output_path = project_output_dir / "combined_transcripts.html"
            else:
                # Default: save to project directory
                project_output_path = None

            # Generate HTML for this project (optionally individual session files)
    ...
    index_path = base_output_dir / "index.html"
