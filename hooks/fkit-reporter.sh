#!/usr/bin/env bash
# FKit Reporter hook — batched delivery with offline queue, retry, and crash recovery
# Auth: Bearer API key via FKIT_API_KEY env var
# Server: FKIT_REPORTER_URL env var
#
# Crash recovery: on session_start, replays missed transcript messages from sessions
# that were closed without the Stop hook firing (power-off, force-quit, etc.)
#
...
SERVER_URL="${FKIT_REPORTER_URL:-http://localhost:3000}"
FKIT_API_KEY="${FKIT_API_KEY:-}"
# Validate API key on startup
if [[ -z "$FKIT_API_KEY" ]]; then
  exit 0  # silently skip — hook not configured yet
fi

# ── Status / manual flush commands ────────────────────────────────────────────
if [[ "${1:-}" == "--status" ]]; then
  QUEUE_FILE="$HOME/.fkit-reporter-queue.jsonl"
  OVERFLOW_FILE="$HOME/.fkit-reporter-queue.overflow"
  ...
  echo "FKit Reporter queue status:"
  echo "  Live queue : $LIVE events  (~$HOME/.fkit-reporter-queue.jsonl)"
  echo "  Overflow   : $OVF events  (~$HOME/.fkit-reporter-queue.overflow)"
...
if [[ "${1:-}" == "--flush" ]]; then
  echo "Manual flush triggered — use 'fkit-reporter.sh --status' to check result."
  # Force flush by resetting last-flush timestamp to 0
  echo 0 > "$HOME/.fkit-reporter-lastflush" 2>/dev/null || true
...
if [[ "${1:-}" == "--update" ]]; then
  ...
  echo "Updating FKit Reporter hook from $SERVER_URL ..."
  if curl -fsSL "$SERVER_URL/hooks/fkit-reporter.sh" -o "$SCRIPT_PATH.tmp"; then
...
# ── Enrich payload: inject entry_uuid, event_timestamp, extract messages ──────
...
try:
    ...
    # ── Catch-up: on session_start, replay missed messages from crashed sessions ──
    if data.get("hook_event_name") == "Notification" and data.get("type") in ("session_start", "resume_session"):
        current_session_id = data.get("session_id", "")
        if state_dir and os.path.isdir(state_dir):
            for state_fname in os.listdir(state_dir):
                if not state_fname.endswith(".last_uuid"):
                    continue
                prev_session_id = state_fname[:-len(".last_uuid")]
                if prev_session_id == current_session_id:
                    continue  # skip current — it is just starting
                state_path = os.path.join(state_dir, state_fname)
                try:
                    with open(state_path) as sf:
                        prev_last_uuid = sf.read().strip()
                except Exception:
                    continue
                if not prev_last_uuid:
                    continue  # no tracked position — nothing to catch up

                # Find transcript JSONL for prev_session_id under ~/.claude/projects/
                projects_dir = os.path.expanduser("~/.claude/projects")
                prev_transcript = None
                if os.path.isdir(projects_dir):
                    for root, dirs, files in os.walk(projects_dir):
                        if prev_session_id + ".jsonl" in files:
                            prev_transcript = os.path.join(root, prev_session_id + ".jsonl")
                            break
                if not prev_transcript:
                    continue

                # Replay: same logic as Stop handler — emit missed messages
                new_messages = []
                found_last = False
                latest_uuid = prev_last_uuid
                seen_uuids = set()
                try:
                    with open(prev_transcript, "r", encoding="utf-8") as f:
                        for line in f:
                            line = line.strip()
                            if not line:
                                continue
                            try:
                                ...
                                if entry_uuid:
                                    seen_uuids.add(entry_uuid)
                                if not found_last:
                                    if entry_uuid == prev_last_uuid:
                                        found_last = True
                                    continue
                                # Capture assistant messages after last_uuid
                                if entry.get("type") == "assistant":
                                    ...
                                    evt_uuid = entry_uuid or str(uuid.uuid4())
                                    if entry_uuid:
                                        latest_uuid = entry_uuid
                                    new_messages.append({
                                        "hook_event_name": "Stop",
                                        "session_id": prev_session_id,
                                        "message": full_text[:10000],
                                        "entry_uuid": evt_uuid,
                                        "event_timestamp": entry.get("timestamp", datetime.datetime.utcnow().isoformat() + "Z"),
                                        "usage": {
                                            "input_tokens": usage.get("input_tokens", 0),
                                            "output_tokens": usage.get("output_tokens", 0),
                                            "cache_creation_input_tokens": usage.get("cache_creation_input_tokens", 0),
                ...
                # If last_uuid not found (very old state file), skip to avoid re-sending everything
                if prev_last_uuid and not found_last and seen_uuids:
                    continue

                if new_messages:
                    for ev in new_messages:
                        print(json.dumps(ev))
                    # Update state file so we do not re-replay on next start
                    if latest_uuid and latest_uuid != prev_last_uuid:
                        try:
  ...
  if [[ -n "$SEND_RESULT" ]]; then
    ...
    while IFS= read -r CHUNK_JSON; do
      ...
      curl -s -o /dev/null -w "%{http_code}" \
        --max-time 15 \
        -X POST "$SERVER_URL/api/reporter/events/batch" \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer $FKIT_API_KEY" \
