...
def _filter_empty_sessions(
    template_messages: List[TemplateMessage],
    sessions: Dict[str, Dict[str, Any]],
    session_order: List[str],
) -> tuple[List[TemplateMessage], Dict[str, Dict[str, Any]], List[str]]:
    """Filter out sessions that have no messages.

    Args:
        template_messages: List of template messages including session headers
        sessions: Dictionary of session metadata
        session_order: List of session IDs in chronological order

    Returns:
        Filtered tuple of (template_messages, sessions, session_order)
    """
    # Count actual messages per session (excluding session headers)
    session_message_counts: Dict[str, int] = {}
    for message in template_messages:
        if not message.is_session_header and message.session_id:
            session_message_counts[message.session_id] = (
                session_message_counts.get(message.session_id, 0) + 1
            )

    # Filter out session headers for sessions with 0 messages
    filtered_messages = [
        msg
        for msg in template_messages
        if not (
            msg.is_session_header and session_message_counts.get(msg.session_id, 0) == 0
        )
    ]

    # Filter sessions dict to only include non-empty sessions
    filtered_sessions = {
        session_id: session_info
        for session_id, session_info in sessions.items()
        if session_message_counts.get(session_id, 0) > 0
    }

    # Filter session_order to only include non-empty sessions
    ...
    return filtered_messages, filtered_sessions, filtered_session_order
...
def generate_html(
    ...
    with log_timing("Filter empty sessions", t_start):
        template_messages, sessions, session_order = _filter_empty_sessions(
            template_messages, sessions, session_order
        )
